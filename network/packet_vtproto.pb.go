...
func (this *NetPacketMessage) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*NetPacketMessage)
	if !ok {
		return false
	}
...
func (m *NetPacketMessage) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: NetPacketMessage: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: NetPacketMessage: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
